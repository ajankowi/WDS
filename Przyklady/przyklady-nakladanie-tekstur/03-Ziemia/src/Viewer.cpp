...
void Viewer::init()
{
  ...
  glGenTextures(1, &Texture4Moon);
  glBindTexture(GL_TEXTURE_2D, Texture4Moon);
  ...
  Texture4Earth =  SOIL_load_OGL_texture(
		"tekstury/earth.jpg",
		SOIL_LOAD_AUTO,
		SOIL_CREATE_NEW_ID,
		SOIL_FLAG_MIPMAPS | SOIL_FLAG_INVERT_Y | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT
...
void Viewer::draw()
{
 ...
 glRotatef( ScnParams.GetAngleX_deg(), 1.2, 0.0, 0 ); // Obracanie kulu wokół osi OX
 GLSphere(0.5);
 ...
 glRotatef( -ScnParams.GetAngleX_deg(), 1.0, 0.0, 0 );
 GLSphere(0.8);
