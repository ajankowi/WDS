...
std::string CreateDataFrame2( int AccX, int AccY )
{
  std::ostringstream  OStrm;
...
int main()
{
  const char* sDataFrame = "X 147 45281 A2";
  int   Num1, Num2;

  std::string  DataFrame = CreateDataFrame(147,45281);

  cout << endl << "   DataFrame: " << DataFrame << endl << endl;;
  ...
  DataFrame = CreateDataFrame2(147,45281);
  byte CRC =  CRC8_DataArray(DataFrame.c_str(),DataFrame.length());
  cout << " Length: " << DataFrame.length() << endl;cout << " CRC: " << static_cast<int>(CRC) << endl;
}
